...
class User {
    ...
    static async register(credentials)
    {
        ...
        const requiredFields = ["email", "fullName", "password", "status", "gender"]
        requiredFields.forEach((field) => {
            if(!credentials.hasOwnProperty(field))
            {
                throw new BadRequestError(`Missing ${field} in request`)
        ...
        const results = await db.query(`
            ...
            VALUES ($1, $2, $3, $4, $5)
            RETURNING id, email, full_name, password, gender, status, created_at
        `, [lowercaseEmail, credentials.fullName, hashedPw, credentials.gender, credentials.status])
